    ...
    if request.method=="POST":
        ...
        if password == password2:
            ...
            if User.objects.filter(username=username).exists():
                messages.error(request,'Böyle bir kullanıcı adı zaten mevcut')
                return redirect('register')
            else:
                if User.objects.filter(email=email).exists():
                    messages.error(request, 'Böyle bir email adresi  zaten mevcut')
                    return redirect('register')
                else:
                    ...
                    # messages.success(request, 'Giriş yaptınızı')
